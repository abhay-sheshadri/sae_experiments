...
def compute_steering_vector(
    ...
    positive_acts = encoder.get_model_residual_acts(positive_examples, batch_size=batch_size)
    negative_acts = encoder.get_model_residual_acts(negative_examples, batch_size=batch_size)
...
def run_soft_prompt_opt(
    ...
    if attack_init:
        ...
        if n_dim == 1:
            ...
            assert torch.all(soft_prompt_mask.sum(-1) == soft_prompt_length), "Soft prompt masks have unequal number of True values across examples"
    ...
    for epoch in tqdm(range(n_epochs)):
        ...
        for batch_input_ids, batch_attention_mask, batch_soft_prompt_mask, batch_target_mask in dataloader:
            ...
            logits = forward_pass_with_interventions(
                ...
                hook_interventions={
                    hook_name: create_hook(batch_soft_prompt_mask)
                },
...
def train_universal_soft_prompt(
    ...
    soft_prompts_tokenized = encoder.tokenizer([soft_prompt_init for i in range(n_examples)], add_special_tokens=False)
    ...
    for i in range(n_examples):
        ...
        sp_mask = [0] * len(before_tokenized['input_ids'][i]) + [1] * len(soft_prompts_tokenized[i]) + [0] * (max_total_len - len(before_tokenized['input_ids'][i]) - len(soft_prompts_tokenized[i]))
        soft_prompt_mask.append(sp_mask)
        # Create target mask
        t_mask = [0] * (len(before_tokenized['input_ids'][i]) + len(soft_prompts_tokenized[i]) + len(after_tokenized['input_ids'][i])) + [1] * len(targets_tokenized['input_ids'][i]) + [0] * padding_length
    ...
    def wrapper_fn(before_soft_prompt, after_soft_prompt, **generation_kwargs):
        # Tokenize the input
        before_tokens = encoder.tokenizer(before_soft_prompt, add_special_tokens=False)['input_ids']
        after_tokens = encoder.tokenizer(after_soft_prompt, add_special_tokens=False)['input_ids']
        # Tokenize the soft prompt init
        soft_prompt_tokens = encoder.tokenizer(soft_prompt_init, add_special_tokens=False)['input_ids']
...
def train_universal_steering_vector(
    ...
    for i in range(n_examples):
        ...
        steer_mask = [0] * (len(before_tokenized['input_ids'][i]) - 1) + [1] * (len(targets_tokenized['input_ids'][i]) + 1) + [0] * padding_length
        steering_mask.append(steer_mask)
        # Create target mask
        t_mask = [0] * len(before_tokenized['input_ids'][i]) + [1] * len(targets_tokenized['input_ids'][i]) + [0] * padding_length
        target_mask.append(t_mask)
    # Convert to tensors
    combined_input_ids = torch.tensor(combined_input_ids)
    combined_attention_masks = torch.tensor(combined_attention_masks)
